...
// echo '<select class="input-control" name="kategori" onchange="document.getElementById(\'prd_name\').value = prdName[this.value]" required>  <option>-Pilih Kategori Foto-</option>';while ($row = mysqli_fetch_array($result)) {  echo ' <option value="' . $row['category_id'] . '">' . $row['category_name'] . '</option>';  
	echo '<select class="input-control" name="kategori" onchange="document.getElementById(\'prd_name\').value = prdName[this.value]" required>';while ($row = mysqli_fetch_array($result)) {  echo ' <option value="' . $row['category_id'] . '">' . $row['category_name'] . '</option>';  
                   ...
                   if(isset($_POST['submit'])){
					   ...
					   if(!in_array($type2, $tipe_diizinkan)){
						  ...
						  echo '<script>alert("Format file tidak diizinkan")</script>';
						   ...
						   $insert = mysqli_query($conn, "INSERT INTO tb_image (category_id, category_name, admin_id, admin_name, image_name, image_description, image, image_status) VALUES (
									  '".$kategori."',
									   '".$nama_ka."',
									   '".$ida."',
									   '".$user."',
